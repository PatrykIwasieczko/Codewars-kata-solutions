...
should ignore the case of the minor words string -- it should behave in the 
...
###Arguments (Other languages)
    ...
    JavaScript/CoffeeScript tests will pass undefined when this argument 
    is unused.
...
function titleCase(title, minorWords) {
    let firstWord = title ? title.split(" ")[0] : "";
    let titleWord = firstWord
        ? firstWord[0].toUpperCase() + firstWord.slice(1).toLowerCase()
        : "";
    let newTitle = [];
    let minor = minorWords ? minorWords.toLowerCase().split(" ") : "";
    let lowerTitle = title ? title.toLowerCase().split(" ") : "";
    for (let i = 1; i < lowerTitle.length; i++) {
        if (minor.includes(lowerTitle[i])) {
            newTitle.push(lowerTitle[i]);
        } else {
            newTitle.push(
                lowerTitle[i][0].toUpperCase() + lowerTitle[i].slice(1)
            );
        }
    }
    return `${titleWord} ${newTitle.join(" ")}`.trim();
}
